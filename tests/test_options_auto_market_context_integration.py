...
def _strong_bullish_ohlcv_rows(count=55):
    return [
        {
...
class OptionsAutoMarketContextIntegrationTests(unittest.TestCase):
    ...
    def test_intended_side_survives_contract_selection_failure(self):
        ...
        self.assertEqual(result["selected_contract"], {})
        self.assertEqual(result["selected_side"], "CE")
        self.assertEqual(result["intended_side"], "CE")
        self.assertEqual(result["final_selected_side"], "WAIT")
        self.assertEqual(result["side_selection"]["source"], "explicit")
        self.assertFalse(result["side_selection"]["contract_selected"])

    def test_selected_side_matches_contract_type_when_contract_is_selected(self):
        result = self.decision(market_context_enabled=False, buy_score_threshold=1)

        self.assertEqual(result["selected_side"], "CE")
        self.assertEqual(result["intended_side"], "CE")
        self.assertEqual(result["final_selected_side"], "CE")
        self.assertEqual(result["selected_contract"]["option_type"], "CE")
        self.assertEqual(result["side_selection"]["selected_contract_type"], "CE")
        self.assertTrue(result["side_selection"]["contract_selected"])

    def test_simple_ohlcv_side_override_is_labelled(self):
        result = evaluate_options_auto_decision(
            mode="PAPER",
            settings={
                "mode": "PAPER",
                "underlying": "NIFTY",
                "entry_dependency_mode": "OHLCV_VOLUME_PROFILE",
                "simple_ohlcv_score_threshold": 20,
                "simple_ohlcv_side_score_threshold": 20,
                "market_cue_alignment_required": False,
                "max_capital_per_trade_pct": 100,
                "max_risk_per_trade_pct": 10,
                "paper_starting_balance": 20000,
            },
            index_history=pd.DataFrame(_strong_bullish_ohlcv_rows()),
            option_candidates=[_candidate()],
            quotes={"1": _quote(), "NIFTY26JUN22500CE": _quote()},
            market_cue_payload={"phase": "LUNCH", "side": "PE", "quote_age_seconds": 0},
            risk_state={},
            account_state={"available_capital": 20000},
            timestamp="2026-06-13T10:30:00",
        )
        ...
        self.assertEqual(result["selected_side"], "CE")
        self.assertEqual(result["side_selection"]["source"], "simple_ohlcv")
        self.assertEqual(result["side_selection"]["explicit_side"], "PE")
        self.assertEqual(result["side_selection"]["simple_ohlcv_side"], "CE")
